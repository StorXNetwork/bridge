...
UsersRouter.prototype._dispatchDelayedActivationEmail = function (user, cb) {
  let self = this;
  self.mailer._options.from = 'support@storx.tech'
  self.mailer.dispatchSendGrid(user.email, 'confirm', {}, function (err) {
    if (err) {
...
UsersRouter.prototype._dispatchActivationEmailSwitch = function (user, cb) {
  console.log(this.config.application.delayedActivation,"this.config.application.delayedActivation")
...
UsersRouter.prototype.subscribeUserToPlan = function (req, res, next) {
  ...
  const config = JSON.parse(require('fs').readFileSync(process.env.HOME + '/.storx-bridge/config/' + process.env.NODE_ENV, 'utf8'));
